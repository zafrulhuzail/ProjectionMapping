...
def detect_cards_yolo(frame, model, conf=0.5):
    """
    Run YOLO on one frame.
    Return a list of 4-corner rectangles in the same style
    your old code used, so the rest of your pipeline still works.
    """
...
def crop_from_corners(frame, corners):
    """
    Take 4 corner points of a YOLO box and crop that rectangle
    directly from the frame.

    No perspective correction here.
    We just cut out the bounding box and resize it to the card size.
    """
    xs = corners[:, 0]
    ys = corners[:, 1]
    ...
    x2 = min(frame.shape[1], int(np.max(xs)))
    y2 = min(frame.shape[0], int(np.max(ys)))
    ...
    crop = cv2.resize(crop, (CARD_WIDTH, CARD_HEIGHT))
    return crop
...
def main():
    ...
    while True:
        ...
        # YOLO detects the card boxes
        card_contours = detect_cards_yolo(frame, model, conf=0.5)

        first_crop = None

        for corners in card_contours:
            cropped = crop_from_corners(frame, corners)
            if cropped is None:
                continue

            if first_crop is None:
                first_crop = cropped.copy()

            label, score = match_card(cropped, references, orb, matcher)
        ...
        if SHOW_DEBUG_WINDOWS:
            if first_crop is not None:
                cv2.imshow("Cropped Card Debug", first_crop)
